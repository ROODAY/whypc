...
function addAnchors(selector) {
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    var anchor = '<a class="anchor-link copylink" href="#' + elementID + '" data-clipboard-text="' + (document.URL + '#' + elementID) + '" title="Click to copy link!"><span class="icon-link"></span></a>';

    $(elements[i]).children(".anchorme").html($(elements[i]).children(".anchorme").html() + anchor);
